...
/*   Updated: 2018/12/06 17:13:17 by lguiller         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			print_sprites(t_all *all, int x)
{
	...
	while ((all->var.i += 1.0f) < dist)
	{
		...
		if (all->rc.map[to_map(all->var.tp.x)][to_map(all->var.tp.y)] == '0' &&
		(all->var.p.x != all->var.save.x || all->var.p.y != all->var.save.y) &&
		(to_map(player.x) != all->var.p.x || to_map(player.y) != all->var.p.y))
		{
